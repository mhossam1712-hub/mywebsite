...
// ssr:false is only valid inside a Client Component — that's why this wrapper exists.
const EyeTestsClient = dynamic(() => import('./EyeTestsClient'), {
  ssr: false,
  loading: () => <EyeTestsSkeleton />,
});

export default function EyeTestsClientLoader({ locale }: { locale: string }) {
  return <EyeTestsClient locale={locale} />;
}
